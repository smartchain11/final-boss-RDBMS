<?php
 namespace App\Libraries; use CodeIgniter\Email\Email; use Config\Email as EmailConfig; class EmailService { private Email $email; public function __construct() { $config = config('Email'); $this->email = new Email($config); $this->email->setFrom($config->fromEmail, $config->fromName); $this->email->setMailType('html'); } private function send(string $to, string $subject, string $body): bool { $this->email->setTo($to); $this->email->setSubject($subject); $this->email->setMessage($body); $result = $this->email->send(); if (! $result) { log_message('error', 'Email send failed: ' . $this->email->printDebugger(['subject', 'to', 'headers'])); } return $result; } private function htmlTemplate(string $title, string $heading, string $content, string $extra = ''): string { return <<<HTML
        <!DOCTYPE html>
        <html>
        ...
        HTML;
} public function sendVerificationCode(string $to, string $code): bool { $subject = 'Verify Your Email ??? Necry OER Portal'; $content = <<<HTML
...
HTML;
return $this->send($to, $subject, $this->htmlTemplate('Verify Email', 'Email Verification', $content)); } public function sendPasswordResetCode(string $to, string $code): bool { $subject = 'Reset Your Password ??? Necry OER Portal'; $content = <<<HTML
...
HTML;
return $this->send($to, $subject, $this->htmlTemplate('Reset Password', 'Password Reset', $content)); } public function sendPurchaseReceipt(string $to, string $userName, string $resourceTitle, float $amount, string $paymentMethod): bool { $subject = 'Purchase Confirmed ??? Necry OER Portal'; $formatted = number_format($amount, 2); $content = <<<HTML
...
HTML;
return $this->send($to, $subject, $this->htmlTemplate('Purchase Receipt', 'Purchase Confirmed', $content)); } public function sendUpgradeConfirmation(string $to, string $userName, string $tier, float $amount): bool { $subject = 'Subscription Upgraded ??? Necry OER Portal'; $formatted = number_format($amount, 2); $tierLabel = str_replace('_', '+ ', $tier); $content = <<<HTML
            <p style="color:#475569;font-size:13px;line-height:1.6;margin:0 0 16px;">Hi <strong>{$userName}</strong>,</p>
...
HTML;
return $this->send($to, $subject, $this->htmlTemplate('Upgrade Confirmed', 'Subscription Upgraded', $content)); } public function sendNewDeviceAlert(string $to, string $userName, string $deviceName, string $ipAddress, string $location): bool { $subject = 'New Sign-In Detected ??? Necry OER Portal'; $content = <<<HTML
            <p style="color:#475569;font-size:13px;line-height:1.6;margin:0 0 16px;">Hi <strong>{$userName}</strong>,</p>
